...
local character          = localPlayer.Character or players.PlayerAdded:Wait().CharacterAdded:Wait()
...
function bot.notify(title, text, duration)
    ...
    print("Notification sent:", title, text)
end
...
function bot.removeBarriers()
    print("Attempting to remove barriers...")
    for _, obj in pairs(workspace:GetDescendants()) do
        if obj:IsA("Part") then
            if obj.Name:match("Barrier") or obj.Name == "NoSpamToolZone" then
                obj:Destroy()
                print("Removed barrier:", obj.Name)
            end
        end
    end
    print("Barrier removal complete.")
end
...
function bot.queueTeleport()
    ...
    -- fix the thing (i hope)
    local encScriptUrl = tostring(getgenv().scriptUrl):gsub('"', '\\"')
    ...
    queue_on_teleport([[
        ...
        local success, response = pcall(function()
            return game:HttpGet("]] .. encScriptUrl .. [[")
        end)
...
function bot.init()
    ...
    if game.PlaceId == 1990228024 then
        print("Correct PlaceId detected. Removing barriers.")
        bot.removeBarriers()
    else
        print("PlaceId does not match. Skipping barrier removal.")
    end
